    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f_img i) : SV_Target
            {
                ...
                ipos.y += floor(_Time.y * _Speed * max(_SpeedMin, _SpeedMax * noise(ipos.x)));
                float charNum = noise(ipos);
                float val = char(fpos, (20. + _Density) * charNum);
                float4 matrixColor = fixed4(_MainColor);
                float4 transparentColor = fixed4(0, 0, 0, 0);
                float4 finalColor = lerp(transparentColor, matrixColor, val);
                return finalColor;
            }
            ENDCG
        }
